  ...
  async startSession(input: CodexAppServerStartSessionInput): Promise<ProviderSession> {
    ...
    try {
      ...
      try {
        ...
        await Effect.logWarning("codex model/list failed", { error: String(error) }).pipe(
          this.runPromise,
        );
      }
      try {
        const accountReadResponse = await this.sendRequest(context, "account/read", {});
        await Effect.logInfo("codex account/read response", {
        ...
        await Effect.logWarning("codex account/read failed", { error: String(error) }).pipe(
          this.runPromise,
        );
      }
