...
export function useScrollBehavior(input: {
  ...
  }, [input.activeThreadId, scheduleStickToBottom]);

  // Auto-scroll on new messages
  useEffect(() => {
    if (!shouldAutoScrollRef.current) return;
    scheduleStickToBottom();
  }, [input.messageCount, scheduleStickToBottom]);

  // Auto-scroll during running phase when timeline entries update
  useEffect(() => {
    if (input.phase !== "running") return;
    if (!shouldAutoScrollRef.current) return;
    scheduleStickToBottom();
  }, [input.phase, scheduleStickToBottom, input.timelineEntries]);
